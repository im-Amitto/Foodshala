...
if (isset($_SESSION["email"]) && isset($_SESSION["userType"]) && $_SESSION["userType"] == "cust") {
  $url = "menu.php";
  redirect($url);
}
require_once 'dbconfig.php';
$name = $email =  $name = $pass = $cpass = $priority =  "";
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (isset($_POST["register"])) {
    ...
    $sql = "SELECT COUNT(*) AS count from cust_login where email = :email_id";
    try {
      $stmt = $DB->prepare($sql);
      ...
      if ($result[0]["count"] > 0) {
        ...
        $sql = "INSERT INTO `cust_login` (`name`, `pass`, `email`, `priority`) VALUES " . "( :name, :pass, :email, :priority)";
        $stmt = $DB->prepare($sql);
        ...
        if ($result > 0) {
          ...
          try {
            ...
            $sql = "Delete from `cust_login` where email = :email";
    ...
    $pass = test_input($_POST["password"]);
    $sql = "SELECT * from cust_login where email = :email";
    try {
      $stmt = $DB->prepare($sql);
      $stmt->bindValue(":email", $email);
      $stmt->execute();
      ...
      if (count($result) > 0) {
        $sql = "SELECT * from cust_login where email = :email and pass= :pass ";
        $stmt = $DB->prepare($sql);
        $stmt->bindValue(":email", $email);
        $stmt->bindValue(":pass", md5($pass));
        $stmt->execute();
        ...
        <form method="post" class="text-center" id="valid">
          ...
          <div class="input-group">
            <label class="radio-inline">
              <input value="veg" type="radio" name="priority" checked>Veg
            </label>
            <label class="radio-inline">
              <input value="nonVeg" type="radio" name="priority">Non Veg
            </label>
          </div><br>
